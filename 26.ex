...
import Enum, only: [reverse: 1]
  ...
  def digits(x, rs, xs) do
    cond do
      # Enum.any?(rs, fn y -> y == rem(10*hd(rs), x) end) && rem(10*hd(rs), x) != 0 -> {x, rs, xs, "recur"}
      Enum.any?(rs, fn y -> y == rem(10*hd(rs), x) end) && rem(10*hd(rs), x) != 0 -> {x, Enum.find_index(rs, fn z -> z == rem(10*hd(rs), x) end) + 1, xs, "recur"}
  ...
  def mainFunc, do: 1..1000 |> Stream.map(fn x -> x |> digits() |> lengthRecur() end)
    # |> Enum.take(10)
    |> Enum.reduce(fn {x1, y1}, {x2, y2} -> if y1 > y2, do: {x1, y1}, else: {x2, y2} end)
